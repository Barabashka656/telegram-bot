...
from keyboards.inline.menu_keyboards.menu_buttoms import shortcut_sources_keyboard, menu_newmsg_back_keyboard,\
...
async def set_shortcut_state(call: types.CallbackQuery):
    ...
    with db:
        shortcut_source_from_db = ShortcutTable.get_or_none(user_id = call.from_user.id)
        if not shortcut_source_from_db:
            await call.message.edit_text("Похоже, вы еще не выбрали сервис для\
...
async def send_shortcut_url(message: types.Message, state: FSMContext):
    ...
    if shortcut_url[0]:
        print(shortcut_url)
        await message.answer(text = "я не могу сократить данную ссылку(", reply_markup=menu_editmsg_back_keyboard)
        await state.finish()
    else:
...
async def set_shortcut_service_in_db(call: types.CallbackQuery):
    ...
    try:
        ...
        with db:
            ...
            await call.message.edit_text(text = f"Вы выбрали сервис для сокращения ссылок {source}\
