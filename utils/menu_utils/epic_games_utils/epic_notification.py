...
async def show_epic_free_notification(user_id: int):
    ...
    for game in games:   
        ...
        if game.start_date and game.end_date:
            ...
            await bot.send_message(chat_id = user_id, text = f"{game.title}\n" + 
    ...
    with db:
        ...
        await bot.send_message(chat_id = user_id, text = f"Следующая рассылка будет в {next_notif_date}", reply_markup=keyboard)
